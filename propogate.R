...
##                     niter=50000,
##                     obj=function(b.next=b.next,s1=s1,s2=s2,b.node=b.node) sum(s1-b.node-(s2-b.next))^2 #objective function
##                     ,...
##                     ) {
##     ##1. get network
##     net<-table(df$nm)
...
##     del<-numeric()
##     s<-sd(c(df$s1,df$s2))
##     mdel.max<-s/100 #this is the convergence criteria. 1/100 of the SD of the scores
##     mdel<-s*100 
##     ##3. iterate
